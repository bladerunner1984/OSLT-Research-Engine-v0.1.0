...
#: OCDS organisation identifiers of the form GB-COH-01234567 carry a Companies House
#: number, which is a strong identifier for entity resolution. GB-CFS- ids are internal
#: to Contracts Finder and are deliberately treated as weak.
_COMPANIES_HOUSE = re.compile(r"^GB-COH-(?P<number>[0-9A-Z]{6,10})$", re.I)
...
def _identifiers_for(party_id: str | None) -> dict[str, str]:
    if not party_id:
        return {}
    match = _COMPANIES_HOUSE.match(party_id.strip())
    if match:
        return {"companies_house": match.group("number").upper(), "ocds_party_id": party_id}
    return {"ocds_party_id": party_id}
...
class ContractsFinderConnector:
    ...
    def _entity(
        ...
        identifiers = _identifiers_for(party_id)
        ...
        *,
        ...
        for release in releases:
            ...
            for award in release.get("awards") or []:
                ...
                start = _parse_date(period.get("startDate")) or award_date
                end = _parse_date(period.get("endDate"))
                if start is None:
                    skip("AWARD_UNDATED")
                    continue
