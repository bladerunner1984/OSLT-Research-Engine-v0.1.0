# OSLT Research Engine v2.4 RC1 - Architecture
...
# OSLT Empirical Research Execution Architecture v2.4 RC1

```text
Declared investigator prior + competing models
                    |
          64 proposition registry
                    |
      Objective / estimand / DAG freeze
                    |
       Adaptive sample-size envelope
                    |
          Minimum field selection
                    |
   +----------------+----------------+
   |                |                |
OPEN/APIs      LICENSED/SAFE     TRE/SDE
   |                |                |
   +----------------+----------------+
                    |
        Provenance + field QA
                    |
   temporal/concept harmonisation
                    |
  dependency + evidence-family graph
                    |
        sealed workstream packs
                    |
  Counter-RAG + rival analyst harness
                    |
 statistical / historical / qualitative
              method kernels
                    |
 falsification + replication + sensitivity
                    |
...
OSLT stores public/licensed evidence where permitted, but controlled person-level data stay in secure environments. The common object crossing environments is the versioned analysis specification and permitted result, not a universal raw-data lake.
